...
def post_comment(request, post_pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # 最终将评论数据保存到数据库，调用数据模型的save方法
            comment.save()

            # 重新向到post的详细页，实际上当redirect函数接受一个模型的实例时，它会调用这个模型实例的get_absolute_url方法，
...
# 那么这个实例必须实现了 get_absolute_url 方法，这样 redirect 会根据 get_absolute_url 方法返回的 URL 值进行重定向。
